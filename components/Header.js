...
import { MdMenu, MdChevronRight, MdLogout } from 'react-icons/md';
// --- Akhir Ikon ---
...
const Header = ({ title, onToggleSidebar, isSidebarExpanded }) => {
  ...
  return (
    <> {/* <-- 1. Tambahkan React Fragment Pembuka */}
      <header className="sticky top-0 z-20 border-b border-gray-100 bg-white/80 backdrop-blur-md">
        <div className="flex h-16 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-4">
            ...
            >
              {isSidebarExpanded ? <MdMenu className="h-6 w-6" /> : <MdChevronRight className="h-6 w-6" />}
            </button>
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              {/* Mobile Avatar */}
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-cyan-700 to-cyan-800 text-xs font-bold text-white shadow-md sm:hidden">
                {(user.displayName || 'U')[0].toUpperCase()}
              </div>
      ...
      {/* 2. Modal dipindahkan ke sini, di luar <header> */}
      {showLogoutConfirm && (
